...
test('Source and bundle should have similar exit codes', async (t) => {
  ...
  try {
    await shell('node ./bundles/babel-preset-current-node-syntax');
    bundleNonzero = false;
  } catch (e) {
    bundleNonzero = true;
  }
  ...
  try {
    await shell('node ./node_modules/babel-preset-current-node-syntax');
    sourceNonzero = false;
  } catch (e) {
    sourceNonzero = true;
  }
  ...
  if (sourceNonzero == bundleNonzero) t.pass();
    ...
    await shell('echo babel-preset-current-node-syntax >> results/failures');
    t.fail(
