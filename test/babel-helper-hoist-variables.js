...
test('Source and bundle should have similar exit codes', async (t) => {
  ...
  try {
    await shell('node ./bundles/babel-helper-hoist-variables');
    bundleNonzero = false;
  } catch (e) {
    bundleNonzero = true;
  }
  ...
  try {
    await shell('node ./node_modules/babel-helper-hoist-variables');
    sourceNonzero = false;
  } catch (e) {
    sourceNonzero = true;
  }
  ...
  if (sourceNonzero == bundleNonzero) t.pass();
    ...
    await shell('echo babel-helper-hoist-variables >> results/failures');
    t.fail(
