...
test('Source and bundle should have similar exit codes', async (t) => {
  ...
  try {
    await shell('node ./bundles/postcss-selector-parser');
    bundleNonzero = false;
  } catch (e) {
    bundleNonzero = true;
  }
  ...
  try {
    await shell('node ./node_modules/postcss-selector-parser');
    sourceNonzero = false;
  } catch (e) {
    sourceNonzero = true;
  }
  ...
  if (sourceNonzero == bundleNonzero) t.pass();
    ...
    await shell('echo postcss-selector-parser >> results/failures');
    t.fail(
