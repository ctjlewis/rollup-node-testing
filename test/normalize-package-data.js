...
test('Source and bundle should have similar exit codes', async (t) => {
  ...
  try {
    await shell('node ./bundles/normalize-package-data');
    bundleNonzero = false;
  } catch (e) {
    bundleNonzero = true;
  }
  ...
  try {
    await shell('node ./node_modules/normalize-package-data');
    sourceNonzero = false;
  } catch (e) {
    sourceNonzero = true;
  }
  ...
  if (sourceNonzero == bundleNonzero) t.pass();
    ...
    await shell('echo normalize-package-data >> results/failures');
    t.fail(
