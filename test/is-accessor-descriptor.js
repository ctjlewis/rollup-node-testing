...
test('Source and bundle should have similar exit codes', async (t) => {
  ...
  try {
    await shell('node ./bundles/is-accessor-descriptor');
    bundleNonzero = false;
  } catch (e) {
    bundleNonzero = true;
  }
  ...
  try {
    await shell('node ./node_modules/is-accessor-descriptor');
    sourceNonzero = false;
  } catch (e) {
    sourceNonzero = true;
  }
  ...
  if (sourceNonzero == bundleNonzero) t.pass();
    ...
    await shell('echo is-accessor-descriptor >> results/failures');
    t.fail(
