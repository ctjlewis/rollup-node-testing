...
var instance = {
  ...
  _failHandshake: function(error) {
    ...
    headers$1.set('Content-Length', Buffer$1.byteLength(error.message, 'utf8'));

    headers$1 = ['HTTP/1.1 400 Bad Request', headers$1.toString(), error.message];
    this._write(Buffer$1.from(headers$1.join('\r\n'), 'utf8'));
...
var instance$3 = {
  read: function() {
    return this.data = this.data || Buffer$3.concat(this._chunks, this.length);
  },
...
var instance$4 = {
  ...
  frame: function(buffer, type, code) {
    ...
    if (buffer instanceof Array)    buffer = Buffer$4.from(buffer);
    ...
    payload = isText ? Buffer$4.from(buffer, 'utf8') : buffer;

    if (code) {
      copy = payload;
      payload = Buffer$4.allocUnsafe(2 + copy.length);
  ...
  _sendFrame: function(frame) {
    var length = frame.length,
        ...
        buffer = Buffer$4.allocUnsafe(offset + length),
...
var Client = function(_url, options) {
  ...
  var uri  = url.parse(this.url),
      auth = uri.auth && Buffer$6.from(uri.auth, 'utf8').toString('base64');
...
var instance$7 = {
    ...
    this._reader.eachByte(function(octet) {
      ...
      switch (this._stage) {
        ...
        case 2:
          if (octet === 0xFF) {
            this._stage = 0;
            message = Buffer$7.from(this._buffer).toString('utf8', 0, this._buffer.length);
  ...
  frame: function(buffer) {
    ...
    var length = Buffer$7.byteLength(buffer),
        frame  = Buffer$7.allocUnsafe(length + 2);
...
var instance$8 = {
  ...
  close: function() {
    if (this.readyState === 3) return false;
    if (this.readyState === 1) this._write(Buffer$8.from([0xFF, 0x00]));
    this.readyState = 3;
    this.emit('close', new base.CloseEvent(null, null));
    return true;
  },
  ...
  _handshakeSignature: function() {
    ...
    var md5    = crypto.createHash('md5'),
        buffer = Buffer$8.allocUnsafe(8 + this.BODY_SIZE);

    buffer.writeUInt32BE(this._keyValues[0], 0);
    buffer.writeUInt32BE(this._keyValues[1], 4);
    Buffer$8.from(this._body).copy(buffer, 8, 0, this.BODY_SIZE);

    md5.update(buffer);
    return Buffer$8.from(md5.digest('binary'), 'binary');
  },
