...
function _prettyFormat() {
  const data = _interopRequireDefault(build);
...
const deprecationMessage = (message, options) => {
  ...
  const name =
    (options.title && options.title.deprecation) || utils.DEPRECATION;
  (0, utils.logValidationWarning)(name, message, comment);
};
...
const unknownOptionWarning = (config, exampleConfig, option, options, path) => {
  const didYouMean = (0, utils.createDidYouMeanMessage)(
    option,
  ...
  const message =
    `  Unknown option ${_chalk().default.bold(
      ...
      (0, utils.format)(config[option])
    )} was found.` +
    (didYouMean && ` ${didYouMean}`) +
    `\n  This is probably a typing mistake. Fixing it will remove this message.`;
  const comment = options.comment;
  const name = (options.title && options.title.warning) || utils.WARNING;
  (0, utils.logValidationWarning)(name, message, comment);
};
...
${formatExamples(option, conditions)}`;
  const comment = options.comment;
  const name = (options.title && options.title.error) || utils.ERROR;
  throw new utils.ValidationError(name, message, comment);
};
...
const createCLIValidationError = (unrecognizedOptions, allowedOptions) => {
  ...
  if (unrecognizedOptions.length === 1) {
    ...
    const didYouMeanMessage = (0, utils.createDidYouMeanMessage)(
      unrecognized,
      Array.from(allowedOptions)
    );
    message =
      `  Unrecognized option ${_chalk().default.bold(
        (0, utils.format)(unrecognized)
      )}.` + (didYouMeanMessage ? ` ${didYouMeanMessage}` : '');
  } else {
    title += 's';
    message =
      `  Following options were not recognized:\n` +
      `  ${_chalk().default.bold((0, utils.format)(unrecognizedOptions))}`;
  }

  return new utils.ValidationError(title, message, comment);
};
...
Object.defineProperty(exports, 'createDidYouMeanMessage', {
  enumerable: true,
  get: function () {
    return utils.createDidYouMeanMessage;
  }
});
...
Object.defineProperty(exports, 'logValidationWarning', {
  enumerable: true,
  get: function () {
    return utils.logValidationWarning;
  }
});
