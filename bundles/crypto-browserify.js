...
var createHmac = crypto$1.createHmac;
...
function nativePBKDF2 (password, salt, iterations, keylen, digest, callback) {
  ...
  return crypto$1.pbkdf2(password, salt, iterations, keylen, digest, callback)
}
...
function nativePBKDF2Sync (password, salt, iterations, keylen, digest) {
  ...
  salt = toBuffer(salt, defaultEncoding_1, 'Salt');
  digest = digest || 'sha1';
  return crypto$1.pbkdf2Sync(password, salt, iterations, keylen, digest)
}

/* istanbul ignore next */
if (!crypto$1.pbkdf2Sync || crypto$1.pbkdf2Sync.toString().indexOf('keylen, digest') === -1) {
...
exports.createCipher = exports.Cipher = crypto$1.createCipher;
exports.createCipheriv = exports.Cipheriv = crypto$1.createCipheriv;
exports.createDecipher = exports.Decipher = crypto$1.createDecipher;
exports.createDecipheriv = exports.Decipheriv = crypto$1.createDecipheriv;
exports.listCiphers = exports.getCiphers = crypto$1.getCiphers;
});

// getDiffieHellman
var DiffieHellmanGroup = crypto$1.DiffieHellmanGroup;
var createDiffieHellmanGroup = crypto$1.createDiffieHellmanGroup;
var getDiffieHellman = crypto$1.getDiffieHellman;

// createDiffieHellman
var createDiffieHellman = crypto$1.createDiffieHellman;
var DiffieHellman = crypto$1.DiffieHellman;
...
var createSign = crypto$1.createSign;
var Sign = crypto$1.Sign;

var createVerify = crypto$1.createVerify;
var Verify = crypto$1.Verify;
...
var createECDH = crypto$1.createECDH;
...
PEMDecoder.prototype.decode = function decode(data, options) {
  ...
  return der_1.prototype.decode.call(this, input, options);
};
...
function copyBuffer(src, target, offset) {
  Buffer$7.prototype.copy.call(src, target, offset);
}
...
function () {
  ...
  _createClass(BufferList, [{
    ...
    value: function concat(n) {
      if (this.length === 0) return Buffer$7.alloc(0);
      var ret = Buffer$7.allocUnsafe(n >>> 0);
      var p = this.head;
      var i = 0;
...
Writable.prototype.write = function (chunk, encoding, cb) {
  ...
  if (isBuf && !Buffer$8.isBuffer(chunk)) {
...
function _isUint8Array$1(obj) {
  return Buffer$a.isBuffer(obj) || obj instanceof OurUint8Array$1;
}
/*<replacement>*/
...
var fixProc = function (okey, password) {
  ...
  if (!match) {
    ...
    decrypted = new Buffer$e(match2[2].replace(/[\r\n]/g, ''), 'base64');
  } else {
    var suite = 'aes' + match[1];
    var iv = Buffer$e.from(match[2], 'hex');
    var cipherText = Buffer$e.from(match[3].replace(/[\r\n]/g, ''), 'base64');
...
function parseKeys (buffer) {
  var password;
  if (typeof buffer === 'object' && !Buffer$f.isBuffer(buffer)) {
    password = buffer.passphrase;
    buffer = buffer.key;
  }
  if (typeof buffer === 'string') {
    buffer = Buffer$f.from(buffer);
  }
...
var mgf = function (seed, len) {
  var t = Buffer$g.alloc(0);
  var i = 0;
  var c;
  while (t.length < len) {
    c = i2ops(i++);
    t = Buffer$g.concat([t, createHash('sha1').update(seed).update(c).digest()]);
  }
  return t.slice(0, len)
};

function i2ops (c) {
  var out = Buffer$g.allocUnsafe(4);
...
function withPublic (paddedMsg, key) {
  return Buffer$h.from(paddedMsg
...
function oaep (key, msg) {
  ...
  var mLen = msg.length;
  var iHash = createHash('sha1').update(Buffer$i.alloc(0)).digest();
  var hLen = iHash.length;
  var hLen2 = 2 * hLen;
  if (mLen > k - hLen2 - 2) {
    throw new Error('message too long')
  }
  var ps = Buffer$i.alloc(k - mLen - hLen2 - 2);
  var dblen = k - hLen - 1;
  var seed = randombytes(hLen);
  var maskedDb = xor(Buffer$i.concat([iHash, ps, Buffer$i.alloc(1, 1), msg], dblen), mgf(seed, dblen));
  var maskedSeed = xor(seed, mgf(maskedDb, hLen));
  return new bn(Buffer$i.concat([Buffer$i.alloc(1), maskedSeed, maskedDb], k))
}
function pkcs1 (key, msg, reverse) {
  ...
  var ps;
  if (reverse) {
    ps = Buffer$i.alloc(k - mLen - 3, 0xff);
  } else {
    ps = nonZero(k - mLen - 3);
  }
  return new bn(Buffer$i.concat([Buffer$i.from([0, reverse ? 1 : 2]), ps, Buffer$i.alloc(1), msg], k))
}
...
var privateDecrypt = function privateDecrypt (privateKey, enc, reverse) {
  ...
  var zBuffer = Buffer$j.alloc(k - msg.length);
  msg = Buffer$j.concat([zBuffer, msg], k);
  if (padding === 4) {
...
function oaep$1 (key, msg) {
  var k = key.modulus.byteLength();
  var iHash = createHash('sha1').update(Buffer$j.alloc(0)).digest();
  var hLen = iHash.length;
  if (msg[0] !== 0) {
    throw new Error('decryption error')
  }
...
if (typeof crypto$1.randomFill === 'function' && typeof crypto$1.randomFillSync === 'function') {
  exports.randomFill = crypto$1.randomFill;
  exports.randomFillSync = crypto$1.randomFillSync;
} else {
