...
function filter(arr) {
  ...
  while (++idx < len) {
    var ele = arr[idx];
    if (kindOf(ele) === 'arguments' || Array.isArray(ele)) {
...
var utils = lazyCache_1(commonjsRequire);
...
commonjsRequire('JSONStream', 'JSONStream');
...
var calculate_1 = createCommonjsModule(function (module) {
...
calculate.group = function(arr, fn) {
  ...
  while (len--) {
    var download = arr[i++];
    var groupArr = utils_1.arrayify(fn(download));
...
calculate.group.before = function(day, arr) {
  var end = utils_1.format(normalizeDate(utils_1.moment(day)));
  var group = [];
  var len = arr.length, i = 0;
  while (len--) {
    var download = arr[i++];
...
calculate.group.last = function(days, arr, init) {
  var today = init ? utils_1.moment.utc(init) : utils_1.moment.utc();
  var start = utils_1.moment.utc(today);
  start.subtract(days, 'days');
  today = utils_1.format(today);
  start = utils_1.format(start);
...
calculate.group.prev = function(days, arr, init) {
  var today = init ? utils_1.moment(init) : utils_1.moment();
  var end = utils_1.moment(today);
...
calculate.group.window = function(days, arr, init) {
  var today = init ? utils_1.moment(init) : normalizeDate(utils_1.moment());
  arr = calculate.group.before(today, arr);
  return calculate.group(arr, function(download) {
    var day = utils_1.moment.utc(download.day);
    var diff = day.diff(today, 'days');
    var period = Math.floor((diff * -1) / days);
    var start = utils_1.moment(today);
    start.subtract((period + 1) * days, 'days');
    return {
      name: period,
      period: utils_1.format(start)
    };
  });
};
