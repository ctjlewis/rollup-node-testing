...
var removeLeadingAndTrailingHTTPWhitespace = string => {
  return string.replace(/^[ \t\n\r]+/, "").replace(/[ \t\n\r]+$/, "");
};

var removeTrailingHTTPWhitespace = string => {
  return string.replace(/[ \t\n\r]+$/, "");
};

var isHTTPWhitespaceChar = char => {
  return char === " " || char === "\t" || char === "\n" || char === "\r";
};

var solelyContainsHTTPTokenCodePoints = string => {
  return /^[-!#$%&'*+.^_`|~A-Za-z0-9]*$/.test(string);
};

var soleyContainsHTTPQuotedStringTokenCodePoints = string => {
...
var utils = {
	removeLeadingAndTrailingHTTPWhitespace: removeLeadingAndTrailingHTTPWhitespace,
	removeTrailingHTTPWhitespace: removeTrailingHTTPWhitespace,
	isHTTPWhitespaceChar: isHTTPWhitespaceChar,
	solelyContainsHTTPTokenCodePoints: solelyContainsHTTPTokenCodePoints,
	soleyContainsHTTPQuotedStringTokenCodePoints: soleyContainsHTTPQuotedStringTokenCodePoints,
	asciiLowercase: asciiLowercase
};

const {
  removeLeadingAndTrailingHTTPWhitespace: removeLeadingAndTrailingHTTPWhitespace$1,
  removeTrailingHTTPWhitespace: removeTrailingHTTPWhitespace$1,
  isHTTPWhitespaceChar: isHTTPWhitespaceChar$1,
  solelyContainsHTTPTokenCodePoints: solelyContainsHTTPTokenCodePoints$1,
...
var parser = input => {
  input = removeLeadingAndTrailingHTTPWhitespace$1(input);
  ...
  if (type.length === 0 || !solelyContainsHTTPTokenCodePoints$1(type)) {
    return null;
  }
  ...
  subtype = removeTrailingHTTPWhitespace$1(subtype);

  if (subtype.length === 0 || !solelyContainsHTTPTokenCodePoints$1(subtype)) {
    return null;
  }
  ...
  while (position < input.length) {
    ...
    while (isHTTPWhitespaceChar$1(input[position])) {
      ++position;
    }
    ...
    if (input[position] === "\"") {
      ...
      parameterValue = removeTrailingHTTPWhitespace$1(parameterValue);
    ...
    if (parameterName.length > 0 &&
        solelyContainsHTTPTokenCodePoints$1(parameterName) &&
        soleyContainsHTTPQuotedStringTokenCodePoints$1(parameterValue) &&
...
const { solelyContainsHTTPTokenCodePoints: solelyContainsHTTPTokenCodePoints$2 } = utils;
...
var serializer = mimeType => {
  ...
  for (let [name, value] of mimeType.parameters) {
    ...
    if (!solelyContainsHTTPTokenCodePoints$2(value) || value.length === 0) {
...
const {
  ...
  soleyContainsHTTPQuotedStringTokenCodePoints: soleyContainsHTTPQuotedStringTokenCodePoints$2
} = utils;
...
var mimeType = class MIMEType {
  ...
  set type(value) {
    value = asciiLowercase$2(String(value));

    if (value.length === 0) {
      throw new Error("Invalid type: must be a non-empty string");
    }
    if (!solelyContainsHTTPTokenCodePoints$3(value)) {
...
class MIMETypeParameters {
  ...
  get(name) {
    name = asciiLowercase$2(String(name));
  ...
  set(name, value) {
    ...
    if (!solelyContainsHTTPTokenCodePoints$3(name)) {
      throw new Error(`Invalid MIME type parameter name "${name}": only HTTP token code points are valid.`);
    }
    if (!soleyContainsHTTPQuotedStringTokenCodePoints$2(value)) {
...
const ctorRegistry = utils$1.ctorRegistrySymbol;
...
exports.is = function is(obj) {
  if (obj) {
    if (utils$1.hasOwn(obj, impl) && obj[impl] instanceof URLSearchParamsImpl_1.implementation) {
      return true;
    }
...
exports.is = function is(obj) {
  if (obj) {
    if (utils$1.hasOwn(obj, impl) && obj[impl] instanceof URLImpl_1.implementation) {
      return true;
    }
    for (const isMixedInto of exports._mixedIntoPredicates) {
      if (isMixedInto(obj)) {
...
const { URL: URL$1, URLSearchParams: URLSearchParams$1 } = webidl2jsWrapper;



const sharedGlobalObject = {};
URL$1.install(sharedGlobalObject);
URLSearchParams$1.install(sharedGlobalObject);
...
var whatwgUrl = {
	URL: URL_1$1,
	URLSearchParams: URLSearchParams_1$1,
	parseURL: parseURL,
...
const { percentDecode: percentDecode$2 } = whatwgUrl;
const { atob: atob$1 } = abab;

var stripLeadingAndTrailingASCIIWhitespace = string => {
  return string.replace(/^[ \t\n\f\r]+/, "").replace(/[ \t\n\f\r]+$/, "");
};

var stringPercentDecode = input => {
  return percentDecode$2(Buffer.from(input, "utf-8"));
};

var isomorphicDecode = input => {
  return input.toString("binary");
};

var forgivingBase64Decode = data => {
  const asString = atob$1(data);
  if (asString === null) {
    return null;
  }
...
var utils$2 = {
	stripLeadingAndTrailingASCIIWhitespace: stripLeadingAndTrailingASCIIWhitespace,
...
const { parseURL, serializeURL } = whatwgUrl;
const {
...
module.exports.fromURLRecord = urlRecord => {
  ...
  let mimeType$1 = "";
  while (position < input.length && input[position] !== ",") {
    mimeType$1 += input[position];
    ++position;
  }
  mimeType$1 = stripLeadingAndTrailingASCIIWhitespace(mimeType$1);
  ...
  const mimeTypeBase64MatchResult = /(.*); *[Bb][Aa][Ss][Ee]64$/.exec(mimeType$1);
  if (mimeTypeBase64MatchResult) {
    ...
    mimeType$1 = mimeTypeBase64MatchResult[1];
  }

  if (mimeType$1.startsWith(";")) {
    mimeType$1 = "text/plain" + mimeType$1;
  }

  let mimeTypeRecord;
  try {
    mimeTypeRecord = new mimeType(mimeType$1);
  } catch (e) {
