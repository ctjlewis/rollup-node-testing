...
PEMDecoder.prototype.decode = function decode(data, options) {
  ...
  var input = new Buffer$1(base64, 'base64');
...
inherits(DERNode$1, base$1.Node);
...
var signature = asn1_1.define('signature', function () {
  this.seq().obj(
    this.key('r').int(),
    this.key('s').int()
  );
...
var toBuffer = function (thing, encoding, name) {
  if (Buffer$b.isBuffer(thing)) {
    return thing
  } else if (typeof thing === 'string') {
    return Buffer$b.from(thing, encoding)
  } else if (ArrayBuffer.isView(thing)) {
    return Buffer$b.from(thing.buffer)
  } else {
...
getCjsExportFromNamespace(aesid$1);
