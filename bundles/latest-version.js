...
var _default = PCancelable;

var CancelError_1 = CancelError;
pCancelable.default = _default;
	...
	const get = async options => {
		...
		const handleRequest = request => {
			...
			try {
				...
				emitError(new RequestError$1(error, options));
			}
		};
		...
		if (options.cache) {
			...
			cacheRequest.once('error', error => {
				if (error instanceof src$3.RequestError) {
					...
					emitError(new CacheError$1(error, options));
				}
			});
