...
function _execa() {
  const data = _interopRequireDefault(execa_1);
...
const findChangedFilesUsingCommand = async (args, cwd) => {
  ...
  return result.stdout
    .split('\n')
    .filter(s => s !== '')
    .map(changedPath => path$1().resolve(cwd, changedPath));
};
...
const adapter = {
  findChangedFiles: async (cwd, options) => {
    ...
    const includePaths = (
      ...
      []
    ).map(absoluteRoot => path$1().normalize(path$1().relative(cwd, absoluteRoot)));
  ...
  getRoot: async cwd => {
    ...
    try {
      ...
      return path$1().resolve(cwd, result.stdout);
    } catch (e) {
      return null;
    }
  }
...
var hg = createCommonjsModule(function (module, exports) {
...
function _throat() {
  const data = _interopRequireDefault(throat);
...
var _git = _interopRequireDefault(git);

var _hg = _interopRequireDefault(hg);
