...
SubQueue.prototype.push = function( task, opt ) {
  ...
  if ( opt.once && lodash.find(this.__queue__, { name: opt.once }) ) {
    return;
  }
...
function Queue( subQueues ) {
  ...
  subQueues = lodash.uniq(subQueues);
...
Queue.prototype.run = function() {
  ...
  this._exec(function() {
    this.running = false;
    if (lodash(this.__queues__).map('__queue__').flatten().value().length === 0) {
      this.emit('end');
    } else {
      this.emit('paused');
    }
