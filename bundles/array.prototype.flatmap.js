...
var ArraySpeciesCreate = function ArraySpeciesCreate(originalArray, length) {
	if (!IsInteger(length) || length < 0) {
		throw new $TypeError$6('Assertion failed: length must be an integer >= 0');
	}
	...
	if (!IsConstructor(C)) {
		throw new $TypeError$6('C must be a constructor');
	}
...
var ToNumber$1 = function ToNumber(argument) {
	...
	if (typeof value === 'symbol') {
		throw new $TypeError$b('Cannot convert a Symbol value to a number');
	}
...
var ToString = function ToString(argument) {
	if (typeof argument === 'symbol') {
		throw new $TypeError$c('Cannot convert a Symbol value to a string');
	}
		...
		if (exists === true) {
			...
			if (typeof mapperFunction !== 'undefined') {
				if (arguments.length <= 6) {
					throw new $TypeError$d('Assertion failed: thisArg is required when mapperFunction is provided');
				}
			...
			if (shouldFlatten) {
				...
				if (targetIndex >= maxSafeInteger) {
					throw new $TypeError$d('index too large');
				}
...
var polyfill = function getPolyfill() {
	return Array.prototype.flatMap || implementation$2;
};
...
var shim = function shimFlatMap() {
	...
	defineProperties_1(
		Array.prototype,
		{ flatMap: polyfill$1 },
		{ flatMap: function () { return Array.prototype.flatMap !== polyfill$1; } }
	);
	return polyfill$1;
};
...
var boundFlatMap = callBind(polyfill$1);
