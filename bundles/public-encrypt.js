...
PEMDecoder.prototype.decode = function decode(data, options) {
  ...
  var input = new Buffer$2(base64, 'base64');
...
DERNode$1.prototype._encodeComposite = function encodeComposite(tag,
  ...
  var header = new Buffer$3(1 + 1 + lenOctets);
  header[0] = encodedTag;
...
DERNode$1.prototype._encodeStr = function encodeStr(str, tag) {
  if (tag === 'bitstr') {
    ...
    var buf = new Buffer$3(str.length * 2);
...
DERNode$1.prototype._encodeInt = function encodeInt(num, values) {
  ...
  if (typeof num !== 'number' && !Buffer$3.isBuffer(num)) {
    var numArray = num.toArray();
    if (!num.sign && numArray[0] & 0x80) {
      numArray.unshift(0);
    }
    num = new Buffer$3(numArray);
  }

  if (Buffer$3.isBuffer(num)) {
    var size = num.length;
    if (num.length === 0)
      size++;

    var out = new Buffer$3(size);
    num.copy(out);
    if (num.length === 0)
      out[0] = 0;
  ...
  return this._createEncoderBuffer(new Buffer$3(out));
};
...
function throwIfNotStringOrBuffer (val, prefix) {
  if (!Buffer$8.isBuffer(val) && typeof val !== 'string') {
...
function HashBase (blockSize) {
  ...
  this._block = Buffer$8.allocUnsafe(blockSize);
...
var fixProc = function (okey, password) {
  ...
  if (!match) {
    ...
    decrypted = new Buffer$b(match2[2].replace(/[\r\n]/g, ''), 'base64');
  } else {
    var suite = 'aes' + match[1];
    var iv = Buffer$b.from(match[2], 'hex');
    var cipherText = Buffer$b.from(match[3].replace(/[\r\n]/g, ''), 'base64');
...
if (typeof crypto$1.privateEncrypt !== 'function') {
  exports.privateEncrypt = browser.privateEncrypt;
} else {
  exports.privateEncrypt = crypto$1.privateEncrypt;
}

if (typeof crypto$1.publicDecrypt !== 'function') {
  exports.publicDecrypt = browser.publicDecrypt;
} else {
